...
<div class="col-sm-9 col-sm-offset-3 col-md-9 col-md-offset-2 main">
	...
	<c:if test="${empty requestScope.discussions}">
		...
		<div class="thumbnail text-info alert alert-info">
			<b>Sorry! No matching discussion found. </b>
		</div>
	</c:if>
	<c:if test="${!empty requestScope.discussions}">
		<c:forEach items="${requestScope.discussions}" var="discussion">
			<div class="row">
				...
				<div class="col-sm-2 text-center">
					<a
						href="${pageContext.request.contextPath}/discussion/view/${discussion.discussionId}">
						<img class="pull-left " width="120px" height="120px" alt=""
						src="${discussion.user.photo}">
					</a>
				</div>
				...
				<div class="col-sm-10">
					...
					<h2 class="text-primary">
						<a
							href="${pageContext.request.contextPath}/discussion/view/${discussion.discussionId}">${discussion.discussionTopic}</a>
					</h2>
					<p class="text-info">${discussion.discussionContent}</p>
					<span class="label label-warning"><b>Comments:
							${discussion.comments.size() }</b></span> <span class="pull-right"> <!-- 		<sec:authorize access="isAuthenticated()"> 
